...
public:
   ...
   template<class... Args>
    static std::shared_ptr<context_t> create(state_t* firstState, Args&&... args) {
        auto context = std::make_shared<context_t>(new typename Utils::start, std::forward<Args>(args)...);
